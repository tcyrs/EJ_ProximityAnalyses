#start TC additions#
setwd("C:/Users/tcyrs/Projects/Proximity Analysis/EJ_ProximityAnalyses")
...
####################################################
#########################   Reclaim facility data
####################################################


## TC notes - added groupby function at end to count instances of name to identify duplicates
facilities <- read_excel("data/reclaim_facility_data/hfc_reclaim_facility_data.xlsx") %>%
  rename(Latitude = Lat,
         Longitude = Long) %>%
  select(Longitude,Latitude,everything()) %>%
  group_by(Name) %>% mutate(N=n())

##TC Notes - loop over and create unique label for duplicate names
facilities$Label <- ""

for (i in 1:nrow(facilities)) {
  if (facilities[i,"N"]>1) {
    facilities[i,"Label"]<-paste(facilities[i,"Name"], facilities[i, "State"])} else {
      facilities[i,"Label"]<-facilities[i, "Name"]
    }
}
...
## TC notes - taking facilities dataframe and transforming to dataframe with geometry attributes##
## CRS (coordinate reference systems) is way of mapping in 2 dimensions##
...
#taking only parts of facilities shapefile that has overlap with urban areas
facilities_sf_urban <- st_intersection(facilities_sf,uac) %>%
  mutate(rural = 0)

#if facilities are in list of urban facilities, 0, otherwise 1 (so that we have list of rural facilities too)
facilities_sf_rural <- facilities_sf %>%
  mutate(rural = fifelse(Label %in% unique(facilities_sf_urban$Label),0,1)) %>%
  as.data.frame() %>%
  select(rural,Label)

#merging everything together based on facility name
facilities_map <- facilities_sf %>%
  left_join(facilities_sf_rural, by = "Label") %>%
  left_join(facilities_lat_lon, by = "Label")

##TC additions to group by regions/divisions

divisions <- divisions()
divisions_t <- divisions %>% st_transform(3488)


fac_map_by_region <- st_intersection(facilities_map, divisions_t)
...
facilities_map_t <- usmap_transform(facilities_lat_lon)

fac_map <- plot_usmap(include=c(.pacific,.west_north_central,.west_region,.west_south_central, .south_region,.north_central_region, .new_england, .mid_atlantic),
                      labels=TRUE,
                      fill = "#C5CFE3",
                      alpha = 0.5) +
...
ggsave("output/H Rule/h_rule_facilities_map.png", fac_map, width = 10, height = 6)
...
# taking first 11 digits of GEOID
...
#keep track points that fall within the urban area shapefile
...
buffer = st_intersection(communities,shp) %>%
  select(GEOID,Tract,Label, fac_bin)
...
facility_demographics_1mi_mid <- merge(facility_demographics_1mi_pre, table_2, by="GEOID") %>%
  select(Label,City,GEOID,sq_miles,rural.x,rural.y,pop,
...
facility_demographics_1mi <- facility_demographics_1mi_mid %>%
  ...
  ungroup() %>%
  select(Label,City,blockgroups_n,sq_miles,pop,pop_sq_mile_1mi,
         rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,
         income,pov50,pov99,total_risk,total_risk_resp) %>%
  distinct()

write.xlsx(facility_demographics_1mi,"output/H Rule/facility_data/h_rule_facility_demographics_1mi.xlsx", overwrite = TRUE)
...
facility_demographics_3mi <- facility_demographics_3mi_mid %>%
  ...
  mutate(pop_sq_mile_3mi = pop/sq_miles,
         rural_bg_pct = signif(sum(rural_blockgroup/blockgroups_n, na.rm=TRUE),2)) %>%
  ungroup() %>%
  select(Label,City,blockgroups_n,sq_miles,pop,pop_sq_mile_3mi,
         rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,
         income,pov50,pov99,total_risk,total_risk_resp) %>%
  distinct()

write.xlsx(facility_demographics_3mi,"output/H Rule/facility_data/h_rule_facility_demographics_3mi.xlsx", overwrite = TRUE)
...
facility_demographics_5mi <- facility_demographics_5mi_mid %>%
  ...
  mutate(pop_sq_mile_5mi = pop/sq_miles,
         rural_bg_pct = signif(sum(rural_blockgroup/blockgroups_n, na.rm=TRUE),2)) %>%
  ungroup() %>%
  select(Label,City,blockgroups_n,sq_miles,pop,pop_sq_mile_5mi,
         rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,
         income,pov50,pov99,total_risk,total_risk_resp) %>%
  distinct()

write.xlsx(facility_demographics_5mi,"output/H Rule/facility_data/h_rule_facility_demographics_5mi.xlsx", overwrite = TRUE)
...
facility_demographics_10mi <- facility_demographics_10mi_mid %>%
  ...
  mutate(pop_sq_mile_10mi = pop/sq_miles,
         rural_bg_pct = signif(sum(rural_blockgroup/blockgroups_n, na.rm=TRUE),2)) %>%
  ungroup() %>%
  select(Label,City,blockgroups_n,sq_miles,pop,pop_sq_mile_10mi,
         rural_facility,rural_bg_pct,white,black,indian,asian,hispanic,
         income,pov50,pov99,total_risk,total_risk_resp) %>%
  distinct()

write.xlsx(facility_demographics_10mi,"output/H Rule/facility_data/h_rule_facility_demographics_10mi.xlsx", overwrite = TRUE)

####################################################
########################  Conduct proximity analysis
####################################################

#read in merged table of ACS/NATA data as dataframe
...
#get bins
bins <- unique(facilities$fac_bin)

#create empty table for each bin to store results in
summary_table_low = summary_table
summary_table_sd_low = summary_table_sd

summary_table_med = summary_table
summary_table_sd_med = summary_table_sd

summary_table_high = summary_table
summary_table_sd_high = summary_table_sd

summary_tables <- list(summary_table_low, summary_table_med, summary_table_high)
summary_tables_sd <- list(summary_table_sd_low, summary_table_sd_med, summary_table_sd_high)

#create large boolean identifying t/f whether GEOID is in buffer zone or not for bin in loop
for (b in 1:length(bins)) {
  local = table$GEOID %in% unique(buffer[buffer$fac_bin == bins[b],]$GEOID)

  #then loop through and pull summary data for GEODS in the boolean object "local"
  for (v in 1:length(comparison_vars)) {
    summary_tables[[b]][v,"Within 1 mile of HFC reclaim facility"] = sum(table$pop[local]*table[local,comparison_vars[v]],na.rm=T)/sum(table$pop[local],na.rm=T)
    a = (table$pop[local]*table[local,comparison_vars[v]])/table$pop[local]
    summary_tables_sd[[b]][v,"Within 1 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  }
}

#repeat for 3mi buffer
for (b in 1:length(bins)) {
  local_3mi = table$GEOID %in% unique(buffer_3mi[buffer_3mi$fac_bin == bins[b],]$GEOID)

  for (v in 1:length(comparison_vars))  {
    summary_tables[[b]][v,"Within 3 miles of HFC reclaim facility"] = sum(table$pop[local_3mi]*table[local_3mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_3mi],na.rm=T)
    a = (table$pop[local_3mi]*table[local_3mi,comparison_vars[v]])/table$pop[local_3mi]
    summary_tables_sd[[b]][v,"Within 3 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  }
}

#repeat for 5mi buffer
for (b in 1:length(bins)) {
  local_5mi = table$GEOID %in% unique(buffer_5mi[buffer_5mi$fac_bin == bins[b],]$GEOID)

  for (v in 1:length(comparison_vars))  {
    summary_tables[[b]][v,"Within 5 miles of HFC reclaim facility"] = sum(table$pop[local_5mi]*table[local_5mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_5mi],na.rm=T)
    a = (table$pop[local_5mi]*table[local_5mi,comparison_vars[v]])/table$pop[local_5mi]
    summary_tables_sd[[b]][v,"Within 5 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  }
}

#repeat for 10mi buffer
for (b in 1:length(bins)) {
  local_10mi = table$GEOID %in% unique(buffer_10mi[buffer_10mi$fac_bin == bins[b],]$GEOID)

  for (v in 1:length(comparison_vars))  {
    summary_tables[[b]][v,"Within 10 miles of HFC reclaim facility"] = sum(table$pop[local_10mi]*table[local_10mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_10mi],na.rm=T)
    a = (table$pop[local_10mi]*table[local_10mi,comparison_vars[v]])/table$pop[local_10mi]
    summary_tables_sd[[b]][v,"Within 10 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
  }
}
...
for (b in 1:length(bins)) {
  summary_tables[[b]][,2:7] = signif(summary_tables[[b]][,2:7],2)
  summary_tables_sd[[b]][,2:7] = signif(summary_tables_sd[[b]][,2:7],2)
}

# export
for (b in 1: length(bins)) {
  list_of_datasets <- list("Means" = summary_tables[[b]], "Standard Deviations" = summary_tables_sd[[b]])
  write.xlsx(list_of_datasets,paste0("output/H Rule/summary_tables/h_summary_tables_", bins[b], ".xlsx"), overwrite = TRUE)

}

####################################################
###############  Run same analsyis but for all in aggregate rather than by bin
####################################################
local = table$GEOID %in% unique(buffer$GEOID)
for (v in 1:length(comparison_vars)) {
  summary_table[v,"Within 1 mile of HFC reclaim facility"] = sum(table$pop[local]*table[local,comparison_vars[v]],na.rm=T)/sum(table$pop[local],na.rm=T)
  a = (table$pop[local]*table[local,comparison_vars[v]])/table$pop[local]
  summary_table_sd[v,"Within 1 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
}

# get the population weighted averages around the reclaim facilities
local_3mi = table$GEOID %in% unique(buffer_3mi$GEOID)
for (v in 1:length(comparison_vars))  {
  summary_table[v,"Within 3 miles of HFC reclaim facility"] = sum(table$pop[local_3mi]*table[local_3mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_3mi],na.rm=T)
  a = (table$pop[local_3mi]*table[local_3mi,comparison_vars[v]])/table$pop[local_3mi]
  summary_table_sd[v,"Within 3 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
}

# get the population weighted averages around the reclaim facilities
local_5mi = table$GEOID %in% unique(buffer_5mi$GEOID)
for (v in 1:length(comparison_vars))  {
  summary_table[v,"Within 5 miles of HFC reclaim facility"] = sum(table$pop[local_5mi]*table[local_5mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_5mi],na.rm=T)
  a = (table$pop[local_5mi]*table[local_5mi,comparison_vars[v]])/table$pop[local_5mi]
  summary_table_sd[v,"Within 5 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
}

# get the population weighted averages around the reclaim facilities
local_10mi = table$GEOID %in% unique(buffer_10mi$GEOID)
for (v in 1:length(comparison_vars))  {
  summary_table[v,"Within 10 miles of HFC reclaim facility"] = sum(table$pop[local_10mi]*table[local_10mi,comparison_vars[v]],na.rm=T)/sum(table$pop[local_10mi],na.rm=T)
  a = (table$pop[local_10mi]*table[local_10mi,comparison_vars[v]])/table$pop[local_10mi]
  summary_table_sd[v,"Within 10 mile of HFC reclaim facility SD"] = sqrt(sum((a-mean(a, na.rm=TRUE))^2/(length(a)-1), na.rm=TRUE))
}
...
write.xlsx(list_of_datasets,"output/H Rule/summary_tables/h_summary_tables_aggregate.xlsx", overwrite = TRUE)
