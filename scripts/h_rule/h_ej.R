...
## TC notes - added groupby function at end to count instances of label to identify duplicates
facilities <- read_excel("data/reclaim_facility_data/reclaim_facility_data.xlsx") %>%
  rename(Latitude = Lat,
         Longitude = Long) %>%
...
fac_map_pacific <- fac_map_by_region[fac_by_region$NAME == 'Pacific',]
...
fac_map <- plot_usmap(include=c(.pacific), #,.west_north_central,.west_region,.west_south_central, .south_region,.north_central_region
                      labels=TRUE,
                      fill = "#C5CFE3",
                      alpha = 0.5) +
  ggrepel::geom_label_repel(data = facilities_map_t,
                            ...
                            segment.color = "#000000", segment.size = 0.3,
                            seed = 1002) +

  geom_point(data = facilities_map_t,
             aes(x = x, y = y),
             color = "#C60404", size = 0.5) +
...
write.xlsx(list_of_datasets,"output/H Rule/summary_tables/h_summary_tables_national.xlsx", overwrite = TRUE)
